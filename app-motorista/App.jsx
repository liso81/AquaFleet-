...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      const perfil = await verificarYActivarMotorista(user.uid, user.phoneNumber);
      if (!perfil) {
  ...
  if (estado === "sin-sesion") {
    return <LoginMotorista onLoginExitoso={() => {}} />;
  }
      ...
      >
        <div className="max-w-sm w-full text-center">
          <div
            ...
            style={{ background: "#F3E3DC" }}
          >
            <Droplet size={20} color={COLORS.clay} />
          </div>
          <h1 className="text-xl font-semibold mb-2" style={{ color: COLORS.ink }}>
            Número no autorizado
          </h1>
          <p className="text-sm mb-6" style={{ color: COLORS.clayDark }}>
            Este número no está registrado como motorista en AquaFleet. Contacta al administrador
            de tu flota para que te dé de alta.
          </p>
          <button
